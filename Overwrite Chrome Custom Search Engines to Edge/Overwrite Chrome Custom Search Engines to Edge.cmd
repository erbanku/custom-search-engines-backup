@echo off
setlocal EnableDelayedExpansion

:: Welcome message
echo WARNING: You should close all Edge and Chrome instances before running this script.
echo This script will overwrite custom search engines in Edge profiles with those exported ones from Chrome profiles.
echo.

set CURRENT_DRIVE=%~d0
set CURRENT_DIR=%~p0
set CHROME_PATH=%HOMEDRIVE%%HOMEPATH%\AppData\Local\Google\Chrome\User Data
set EDGE_PATH=%HOMEDRIVE%%HOMEPATH%\AppData\Local\Microsoft\Edge\User Data
set TEMP_SQL_SCRIPT=%TEMP%\sync_sql_script
set TEMP_DB=%TEMP%\temp_web_data

if "%1"=="" (
    set BASE_DESTINATION=%CURRENT_DRIVE%%CURRENT_DIR%Chromekeywords
) else (
    set BASE_DESTINATION=%~f1
)

pushd

:: List available Chrome profiles
set chrome_profile_count=0
echo Available Chrome profiles:
for /d %%P in ("%CHROME_PATH%\*") do (
    if exist "%%P\Web Data" (
        set /a chrome_profile_count+=1
        echo !chrome_profile_count!: %%~nxP
        set "chrome_profile_!chrome_profile_count!=%%~nxP"
    )
)

:: List available Edge profiles
set edge_profile_count=0
echo Available Edge profiles:
for /d %%P in ("%EDGE_PATH%\*") do (
    if exist "%%P\Web Data" (
        set /a edge_profile_count+=1
        echo !edge_profile_count!: %%~nxP
        set "edge_profile_!edge_profile_count!=%%~nxP"
    )
)

:: Get user selection for Chrome and Edge profile pairs
:select_profiles
set /p "chrome_selection=Enter Chrome profile number to export (or 'done' to finish after overwrite complete): "
if /i "!chrome_selection!"=="done" goto end

:: Validate Chrome selection
if "!chrome_selection!"=="" (
    echo Error: No Chrome profile selection entered. Please try again.
    goto select_profiles
)

:: Check if Chrome selection is a valid number and within range
if !chrome_selection! lss 1 (
    echo Error: Please enter a valid Chrome profile number (1 to !chrome_profile_count!).
    goto select_profiles
)
if !chrome_selection! gtr !chrome_profile_count! (
    echo Error: Please enter a valid Chrome profile number (1 to !chrome_profile_count!).
    goto select_profiles
)

set /p "edge_selection=Enter Edge profile number to import to: "

:: Validate Edge selection  
if "!edge_selection!"=="" (
    echo Error: No Edge profile selection entered. Please try again.
    goto select_profiles
)

:: Check if Edge selection is a valid number and within range
if !edge_selection! lss 1 (
    echo Error: Please enter a valid Edge profile number (1 to !edge_profile_count!).
    goto select_profiles
)
if !edge_selection! gtr !edge_profile_count! (
    echo Error: Please enter a valid Edge profile number (1 to !edge_profile_count!).
    goto select_profiles
)

:: Process the selected profiles
set "chrome_profile=!chrome_profile_%chrome_selection%!"
set "edge_profile=!edge_profile_%edge_selection%!"

:: Sanitize profile names for filename (replace problematic characters)
set "safe_chrome_profile=!chrome_profile: =_!"
set "safe_chrome_profile=!safe_chrome_profile:(=_!"
set "safe_chrome_profile=!safe_chrome_profile:)=_!"
set "safe_chrome_profile=!safe_chrome_profile:[=_!"
set "safe_chrome_profile=!safe_chrome_profile:]=_!"

set DESTINATION=%BASE_DESTINATION%_!safe_chrome_profile!.sql
set DESTINATION=!DESTINATION:\=/!

echo Exporting Chrome keywords from !chrome_profile! to !DESTINATION!...
cd /D "%CHROME_PATH%\!chrome_profile!"

rem Copy database to temp location to avoid locks
copy /Y "Web Data" "%TEMP_DB%" >nul

echo .output "!DESTINATION!" > %TEMP_SQL_SCRIPT%
echo .dump keywords >> %TEMP_SQL_SCRIPT%
"%CURRENT_DRIVE%%CURRENT_DIR%\sqlite3.exe" -init %TEMP_SQL_SCRIPT% "%TEMP_DB%" .exit

if exist "!DESTINATION!" (
    for %%F in ("!DESTINATION!") do if %%~zF gtr 0 (
        echo Successfully exported !chrome_profile!
    ) else (
        echo Failed to export !chrome_profile! - empty file
        pause
    )
) else (
    echo Failed to export !chrome_profile! - file not created
    pause
)

rem Clean up temp files
del %TEMP_SQL_SCRIPT% 2>nul
del %TEMP_DB% 2>nul

set SOURCE=%BASE_DESTINATION%_!safe_chrome_profile!.sql
set SOURCE=!SOURCE:\=/!

if exist "!SOURCE!" (
    echo Importing Edge keywords to !edge_profile! from !SOURCE!...
    cd /D "%EDGE_PATH%\!edge_profile!"
    
    rem Copy database to temp location to avoid locks
    copy /Y "Web Data" "%TEMP_DB%" >nul
    
    "%CURRENT_DRIVE%%CURRENT_DIR%\sqlite3.exe" "%TEMP_DB%" < "!SOURCE!"
    
    rem Replace the original database with the updated one
    copy /Y "%TEMP_DB%" "Web Data" >nul
    
    echo Successfully imported keywords to !edge_profile!
    
    rem Clean up temp files
    del %TEMP_DB% 2>nul
) else (
    echo Source file for !edge_profile! not found: !SOURCE!
    pause
)

goto select_profiles

:end
popd
echo Operation complete.
pause